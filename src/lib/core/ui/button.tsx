...
export type XButtonProps = VariantProps<typeof buttonVariants> &
  ButtonRootProps;

export const XButton: Component<XButtonProps> = (props) => {
  const [local, pass] = splitProps(props, ["variant", "size", "class"]);
  return (
    <Button.Root
      classList={{
        [buttonVariants({ variant: local.variant, size: local.size })]: true,
        [local.class ?? ""]: true,
      }}
      {...pass}
    />
